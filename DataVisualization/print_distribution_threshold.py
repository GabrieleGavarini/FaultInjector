...
top_non_faulty_values = faulty_inference_results[~faulty_inference_results[fault_metric]].top_1.sort_values(ascending=False).replace([np.inf, -np.inf], np.nan).dropna().reset_index(drop=True)
...
top_faulty_values = faulty_inference_results[faulty_inference_results[fault_metric]].top_1.sort_values(ascending=False).replace([np.inf, -np.inf], np.nan).dropna().reset_index(drop=True)
